-- Phase F hotfix: paywall helper must be callable from authenticated RPC paths.
--
-- 0019 introduced public.user_has_active_entitlement() and then revoked EXECUTE
-- from authenticated. That broke submit_digest (and could break pro test starts)
-- because SECURITY INVOKER RPCs still need execute rights for helper functions
-- they call. Keep direct calls privacy-safe by returning false when a user asks
-- about any user_id other than their own.
...
as $$
  select case
    when auth.uid() is not null and auth.uid() <> p_user_id then false
    else exists (
      select 1
...
revoke execute on function public.user_has_active_entitlement(uuid)
  from public, anon;
grant execute on function public.user_has_active_entitlement(uuid)
  to authenticated;
...
begin
  ...
  if coalesce(v_test_is_pro, false) then
    if not public.user_has_active_entitlement(v_uid) then
...
revoke execute on function public.submit_digest(date, jsonb) from anon, public;
